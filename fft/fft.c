...
#include <stdint.h>
//精度0.0001弧度
#define SQRT(X) SQRT_newton(X)
float SQRT_newton(float num) {
    if (num < 0) return -1; // 处理负数输入，返回错误值
    if (num == 0) return 0; // 处理特殊值

    float x = num / 2.0;    // 初始猜测值
    float epsilon = 1e-3; // 精度要求
    while (1) {
        float next = 0.5 * (x + num / x);
        if (fabs(next - x) < epsilon) // 判断收敛条件
            break;
        x = next;
    }
    return x;
